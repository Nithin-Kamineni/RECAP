...
**140 invariants over 156 refusal sites.** tier 1 (PARSE): 29, tier 2 (IMPOSSIBLE): 107, tier 3 (COUPLING): 3, tier 4 (DERIVED): 1
...
| `not-a-key-value-list` | `ConfigError` | an ECC_LAYERS per-model entry that is not `model=layer layer` | `settings/env.py:168`<br>`settings/env.py:195`<br>`settings/env.py:247`<br>`settings/env.py:182` |
| `not-a-key-value-table` | `ConfigError` | an `ECC_*_LIST` entry that is not `key=number` | `settings/env.py:91` |
| `not-a-number` | `ConfigError` | a float knob that is not a number | `settings/env.py:65`<br>`settings/env.py:126` |
| `not-an-integer` | `ConfigError` | an integer knob that is not an integer | `settings/env.py:54` |
| `one-value-not-a-list` | `ConfigError` | a list where ONE value is meant (only the swept axis takes a list) | `settings/env.py:207`<br>`settings/env.py:235` |
| `unknown-approach` | `ConfigError` | ECC_APPROACHES naming an arm that does not exist | `config.py:223` |
| `unknown-arch-fidelity` | `ConfigError` | ECC_ARCH_FIDELITY naming no known fidelity | `config.py:613` |
| `unknown-capacity-scope` | `ConfigError` | ECC_WEIGHT_CAPACITY_SCOPE naming no known scope | `config.py:573` |
| `unknown-classify` | `ConfigError` | ECC_CLASSIFY that is neither `instances` nor `name` | `config.py:563` |
| `unknown-encoder-granularity` | `ConfigError` | ECC_RECON_ENCODER_GRANULARITY naming no known granularity | `config.py:427` |
| `unknown-encoder-site` | `ConfigError` | ECC_RECON_ENCODER_SITE naming no known site | `config.py:462` |
| `unknown-experiment` | `ConfigError` | ECC_EXPERIMENT naming no known experiment | `config.py:186` |
| `unknown-format` | `ConfigError` | ECC_FORMATS naming a format matplotlib is not asked for | `config.py:632` |
| `unknown-knob` | `ConfigError` | `cfg.with_()` naming a knob no settings group declares | `config.py:738` |
...
| `unknown-opt-metric` | `ConfigError` | ECC_OPT_METRIC naming no known objective | `config.py:617` |
| `unknown-palette` | `ConfigError` | ECC_PALETTE that is neither `house` nor `cvd` | `config.py:628` |
| `unknown-parity-grouping` | `ConfigError` | ECC_PARITY_GROUPING naming no known grouping | `config.py:403` |
...
| `unknown-recon-packing` | `ConfigError` | ECC_RECON_PACKING naming no known packing | `config.py:423` |
| `unknown-sweep` | `ConfigError` | ECC_SWEEP naming no known axis | `config.py:192` |
| `unknown-victory-scaling` | `ConfigError` | ECC_VICTORY_SCALING naming no known scaling | `config.py:621` |
| `values-unknown-sweep` | `SystemExit` | `--values` for a sweep that has no knob | `__main__.py:84` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `acc-bits-not-narrower` | `ConfigError` | an accumulator narrower than the operands it accumulates | `config.py:398` |
| `accelergy-wrote-no-ert` | `SystemExit` | Accelergy producing no ERT/ART at all | `toolchain/ert.py:352` |
| `activation-bits-positive` | `ConfigError` | ECC_ACTIVATION_BITS <= 0 | `config.py:390` |
...
| `capacity-scale-positive` | `ConfigError` | ECC_WEIGHT_CAPACITY_SCALE <= 0 | `config.py:566` |
| `clock-positive` | `ConfigError` | a declared clock rate that is not positive | `arch/design.py:309`<br>`config.py:1358` |
| `const-arch-empty` | `ConfigError` | a held architecture axis with no ECC_CONST_ARCH | `config.py:334` |
| `const-model-empty` | `ConfigError` | a held model axis with no ECC_CONST_MODEL | `config.py:346` |
| `datawidth-le-weight-bits` | `ConfigError` | a REDUCED weight wider than the full one | `config.py:608` |
| `datawidth-levels-unknown` | `ValueError` | ECC_WEIGHT_DATAWIDTH_LEVELS naming a level the design has not got | `arch/patch.py:792` |
| `datawidth-positive` | `ConfigError` | ECC_WEIGHT_DATAWIDTH below 1 bit | `config.py:600` |
| `dc-clock-mismatch` | `SystemExit` | a DC datapath entry measured at another clock period | `physics/recon_dc.py:128` |
| `depth-ladder-empty` | `ConfigError` | ECC_SWEEP=area with an empty ECC_DEPTH_SWEEP_SCALES -- an x axis with no points | `config.py:312` |
| `depth-scale-positive` | `ConfigError` | ECC_WEIGHT_DEPTH_SCALE <= 0 | `config.py:582`<br>`config.py:318` |
...
| `dram-static-terms-nonnegative` | `ConfigError` | a negative DRAM background or refresh term | `config.py:472` |
...
| `ert-arm-one-arch` | `ConfigError` | ECC_RECON_ERT_ARM with more than one architecture | `config.py:652` |
...
| `need-k-lt-n` | `ConfigError` | a BCH code with K >= N | `config.py:382` |
| `need-weak-k-lt-n` | `ConfigError` | a weak code with WEAK_K >= WEAK_N | `config.py:560` |
| `negative-price` | `ConfigError` | a NEGATIVE per-bit or per-MAC price | `config.py:449` |
...
| `no-cnn-transformer-mix` | `ConfigError` | one sweep mixing CNNs and transformers (two workload files) | `config.py:518` |
...
| `panels-need-panel-models` | `ConfigError` | ECC_EXPERIMENT=panels with no ECC_PANEL_MODELS | `config.py:364` |
| `panels-nothing-to-plot` | `SystemExit` | a panel figure with no group in any panel | `report/panels.py:405` |
...
| `recon-needs-an-arch` | `ConfigError` | the placement study with no architecture | `config.py:481` |
...
| `recon-one-model` | `ConfigError` | the placement study on more than one model | `config.py:498` |
...
| `sweep-archs-empty` | `ConfigError` | ECC_SWEEP=arch with no architectures | `config.py:329` |
| `sweep-has-no-figure` | `ConfigError` | a sweep or panel FIGURE on an axis in settings/run.py's NO_FIGURE_SWEEPS (EMPTY since 2026-09-15: area got a renderer) | `config.py:297` |
| `sweep-k-lt-n` | `ConfigError` | an ECC_SWEEP_KS entry that is not below N | `config.py:377` |
| `sweep-ks-empty` | `ConfigError` | ECC_SWEEP=bch with no codes | `config.py:372` |
| `sweep-models-empty` | `ConfigError` | ECC_SWEEP=model with no models | `config.py:341` |
| `sweep-nothing-to-plot` | `SystemExit` | a sweep figure with no group | `report/sweep.py:422` |
...
| `unknown-ert-arm` | `ConfigError` | ECC_RECON_ERT_ARM naming no arm of this design | `config.py:659` |
| `victory-ge-1` | `ConfigError` | ECC_VICTORY below 1 | `config.py:625` |
| `weight-bits-positive` | `ConfigError` | ECC_WEIGHT_BITS <= 0 | `config.py:386` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `panels-needs-arch-or-model-sweep` | `ConfigError` | a panel layout that would vary two axes at once | `config.py:358` |
| `recon-no-split-read-write` | `ConfigError` | ECC_SPLIT_READ_WRITE=1 inside the placement study | `config.py:504` |
| `rerun-needs-the-mapper` | `ConfigError` | ECC_RERUN_OPTIMISER=1 together with a knob that never invokes Timeloop | `config.py:554` |

## Tier 4 — DERIVED

| id | raises | refuses | where |
|---|---|---|---|
| `zero-price` | `ConfigError` | a ZERO per-bit or per-MAC price | `config.py:455` |
