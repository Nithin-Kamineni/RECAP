...
**132 invariants over 138 refusal sites.** tier 1 (PARSE): 26, tier 2 (IMPOSSIBLE): 99, tier 3 (COUPLING): 4, tier 4 (DERIVED): 3
...
| `unknown-approach` | `ConfigError` | ECC_APPROACHES naming an arm that does not exist | `config.py:196` |
| `unknown-arch-fidelity` | `ConfigError` | ECC_ARCH_FIDELITY naming no known fidelity | `config.py:474` |
| `unknown-capacity-scope` | `ConfigError` | ECC_WEIGHT_CAPACITY_SCOPE naming no known scope | `config.py:434` |
| `unknown-classify` | `ConfigError` | ECC_CLASSIFY that is neither `instances` nor `name` | `config.py:424` |
| `unknown-encoder-granularity` | `ConfigError` | ECC_RECON_ENCODER_GRANULARITY naming no known granularity | `config.py:314` |
| `unknown-encoder-site` | `ConfigError` | ECC_RECON_ENCODER_SITE naming no known site | `config.py:349` |
| `unknown-experiment` | `ConfigError` | ECC_EXPERIMENT naming no known experiment | `config.py:184` |
| `unknown-format` | `ConfigError` | ECC_FORMATS naming a format matplotlib is not asked for | `config.py:493` |
| `unknown-knob` | `ConfigError` | `cfg.with_()` naming a knob no settings group declares | `config.py:611` |
| `unknown-opt-metric` | `ConfigError` | ECC_OPT_METRIC naming no known objective | `config.py:478` |
| `unknown-palette` | `ConfigError` | ECC_PALETTE that is neither `house` nor `cvd` | `config.py:489` |
| `unknown-parity-grouping` | `ConfigError` | ECC_PARITY_GROUPING naming no known grouping | `config.py:290` |
| `unknown-recon-packing` | `ConfigError` | ECC_RECON_PACKING naming no known packing | `config.py:310` |
| `unknown-sweep` | `ConfigError` | ECC_SWEEP naming no known axis | `config.py:190` |
| `unknown-victory-scaling` | `ConfigError` | ECC_VICTORY_SCALING naming no known scaling | `config.py:482` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `acc-bits-not-narrower` | `ConfigError` | an accumulator narrower than the operands it accumulates | `config.py:285` |
| `accelergy-wrote-no-ert` | `SystemExit` | Accelergy producing no ERT/ART at all | `toolchain/ert.py:287` |
| `activation-bits-positive` | `ConfigError` | ECC_ACTIVATION_BITS <= 0 | `config.py:277` |
| `approaches-empty` | `ConfigError` | ECC_APPROACHES empty -- nothing to compare | `config.py:200` |
...
| `capacity-scale-positive` | `ConfigError` | ECC_WEIGHT_CAPACITY_SCALE <= 0 | `config.py:427` |
| `clock-positive` | `ConfigError` | a declared clock rate that is not positive | `arch/design.py:309`<br>`config.py:1087` |
| `const-arch-empty` | `ConfigError` | a held architecture axis with no ECC_CONST_ARCH | `config.py:221` |
| `const-model-empty` | `ConfigError` | a held model axis with no ECC_CONST_MODEL | `config.py:233` |
| `datawidth-le-weight-bits` | `ConfigError` | a REDUCED weight wider than the full one | `config.py:469` |
| `datawidth-levels-unknown` | `ValueError` | ECC_WEIGHT_DATAWIDTH_LEVELS naming a level the design has not got | `arch/patch.py:792` |
| `datawidth-positive` | `ConfigError` | ECC_WEIGHT_DATAWIDTH below 1 bit | `config.py:461` |
| `dc-clock-mismatch` | `SystemExit` | a DC datapath entry measured at another clock period | `physics/recon_dc.py:128` |
| `depth-scale-positive` | `ConfigError` | ECC_WEIGHT_DEPTH_SCALE <= 0 | `config.py:443` |
...
| `dram-static-terms-nonnegative` | `ConfigError` | a negative DRAM background or refresh term | `config.py:359` |
...
| `ert-arm-one-arch` | `ConfigError` | ECC_RECON_ERT_ARM with more than one architecture | `config.py:516` |
...
| `ert-toll-mismatch` | `SystemExit` | an evaluator-charged toll that is not the stats-side amount | `study/placement_study.py:289` |
| `figure-draws-the-total` | `SystemExit` | a figure whose bars do not add up to the result's total | `report/recon_view.py:101` |
...
| `missing-path` | `SystemExit` | a required input file that is not there | `paths.py:281` |
| `narrow-once` | `ValueError` | the mapper AND the evaluator both narrowing (it squares the saving) | `study/narrowing.py:130` |
| `need-k-lt-n` | `ConfigError` | a BCH code with K >= N | `config.py:269` |
| `need-weak-k-lt-n` | `ConfigError` | a weak code with WEAK_K >= WEAK_N | `config.py:421` |
| `negative-price` | `ConfigError` | a NEGATIVE per-bit or per-MAC price | `config.py:336` |
...
| `no-boundary-mapped` | `SystemExit` | ECC_RECON_ERT_AWARE=1 with not one boundary mapped | `study/placement_study.py:187` |
| `no-cnn-transformer-mix` | `ConfigError` | one sweep mixing CNNs and transformers (two workload files) | `config.py:403` |
| `no-dilatable-level` | `SystemExit` | Task 4 on a design with no weight level a dilation can touch | `study/dilated_view.py:108` |
| `no-weight-path` | `SystemExit` | a placement study on a design that declares no weight path | `report/recon_view.py:373` |
...
| `panels-need-panel-models` | `ConfigError` | ECC_EXPERIMENT=panels with no ECC_PANEL_MODELS | `config.py:251` |
...
| `recon-needs-an-arch` | `ConfigError` | the placement study with no architecture | `config.py:368` |
| `recon-nothing-collected` | `SystemExit` | a panelled placement study with a panel that collected nothing | `report/recon_view.py:411` |
| `recon-one-model` | `ConfigError` | the placement study on more than one model | `config.py:384` |
...
| `sweep-archs-empty` | `ConfigError` | ECC_SWEEP=arch with no architectures | `config.py:216` |
| `sweep-k-lt-n` | `ConfigError` | an ECC_SWEEP_KS entry that is not below N | `config.py:264` |
| `sweep-ks-empty` | `ConfigError` | ECC_SWEEP=bch with no codes | `config.py:259` |
| `sweep-models-empty` | `ConfigError` | ECC_SWEEP=model with no models | `config.py:228` |
...
| `unknown-ert-arm` | `ConfigError` | ECC_RECON_ERT_ARM naming no arm of this design | `config.py:523` |
| `unknown-placement` | `SystemExit` | ECC_RECON_PLACEMENTS naming a boundary the design does not define | `report/recon_view.py:396` |
| `victory-ge-1` | `ConfigError` | ECC_VICTORY below 1 | `config.py:486` |
| `weight-bits-positive` | `ConfigError` | ECC_WEIGHT_BITS <= 0 | `config.py:273` |
| `weight-path-drifted` | `SystemExit` | a weight path and a placement list that have drifted apart | `report/recon_view.py:383` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `ert-arm-needs-optimiser` | `ConfigError` | ECC_RECON_ERT_AWARE=1 without RECON_OPTIMIZER=True | `config.py:498` |
| `panels-needs-arch-or-model-sweep` | `ConfigError` | a panel layout that would vary two axes at once | `config.py:245` |
| `recon-no-split-read-write` | `ConfigError` | ECC_SPLIT_READ_WRITE=1 inside the placement study | `config.py:389` |
| `rerun-needs-the-mapper` | `ConfigError` | ECC_RERUN_OPTIMISER=1 together with a knob that never invokes Timeloop | `config.py:415` |

## Tier 4 — DERIVED

| id | raises | refuses | where |
|---|---|---|---|
| `derived-datawidth` | `ConfigError` | ECC_WEIGHT_DATAWIDTH set beside an arm that also derives q | `config.py:528` |
| `derived-datawidth-levels` | `ConfigError` | ECC_WEIGHT_DATAWIDTH_LEVELS set beside an arm that also derives them | `config.py:535` |
| `zero-price` | `ConfigError` | a ZERO per-bit or per-MAC price | `config.py:342` |
