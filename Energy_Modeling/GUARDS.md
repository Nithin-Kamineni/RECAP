...
| `unknown-arch-fidelity` | `ConfigError` | ECC_ARCH_FIDELITY naming no known fidelity | `config.py:521` |
| `unknown-capacity-scope` | `ConfigError` | ECC_WEIGHT_CAPACITY_SCOPE naming no known scope | `config.py:481` |
| `unknown-classify` | `ConfigError` | ECC_CLASSIFY that is neither `instances` nor `name` | `config.py:471` |
| `unknown-encoder-granularity` | `ConfigError` | ECC_RECON_ENCODER_GRANULARITY naming no known granularity | `config.py:335` |
...
| `unknown-recon-packing` | `ConfigError` | ECC_RECON_PACKING naming no known packing | `config.py:331` |
| `unknown-sweep` | `ConfigError` | ECC_SWEEP naming no known axis | `config.py:190` |
| `unknown-victory-scaling` | `ConfigError` | ECC_VICTORY_SCALING naming no known scaling | `config.py:529` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `acc-bits-not-narrower` | `ConfigError` | an accumulator narrower than the operands it accumulates | `config.py:306` |
| `accelergy-wrote-no-ert` | `SystemExit` | Accelergy producing no ERT/ART at all | `toolchain/ert.py:287` |
| `activation-bits-positive` | `ConfigError` | ECC_ACTIVATION_BITS <= 0 | `config.py:298` |
...
| `datawidth-positive` | `ConfigError` | ECC_WEIGHT_DATAWIDTH below 1 bit | `config.py:508` |
| `dc-clock-mismatch` | `SystemExit` | a DC datapath entry measured at another clock period | `physics/recon_dc.py:128` |
| `depth-scale-positive` | `ConfigError` | ECC_WEIGHT_DEPTH_SCALE <= 0 | `config.py:490` |
...
| `dram-static-terms-nonnegative` | `ConfigError` | a negative DRAM background or refresh term | `config.py:380` |
...
| `ert-arm-one-arch` | `ConfigError` | ECC_RECON_ERT_ARM with more than one architecture | `config.py:560` |
...
| `need-k-lt-n` | `ConfigError` | a BCH code with K >= N | `config.py:290` |
| `need-weak-k-lt-n` | `ConfigError` | a weak code with WEAK_K >= WEAK_N | `config.py:468` |
| `negative-price` | `ConfigError` | a NEGATIVE per-bit or per-MAC price | `config.py:357` |
...
| `no-cnn-transformer-mix` | `ConfigError` | one sweep mixing CNNs and transformers (two workload files) | `config.py:426` |
...
| `recon-needs-an-arch` | `ConfigError` | the placement study with no architecture | `config.py:389` |
| `recon-nothing-collected` | `SystemExit` | a panelled placement study with a panel that collected nothing | `report/recon_view.py:408` |
| `recon-one-model` | `ConfigError` | the placement study on more than one model | `config.py:406` |
| `stacks-refused` | `SystemExit` | a figure with no stacks to draw | `study/common.py:207` |
| `standard-file-missing` | `SystemExit` | the apples-to-apples contract file being absent | `arch/load.py:391` |
| `sweep-archs-empty` | `ConfigError` | ECC_SWEEP=arch with no architectures | `config.py:237` |
| `sweep-has-no-figure` | `ConfigError` | a sweep or panel FIGURE on an axis that holds all three lists (fix, area) | `config.py:226` |
| `sweep-k-lt-n` | `ConfigError` | an ECC_SWEEP_KS entry that is not below N | `config.py:285` |
| `sweep-ks-empty` | `ConfigError` | ECC_SWEEP=bch with no codes | `config.py:280` |
...
| `victory-ge-1` | `ConfigError` | ECC_VICTORY below 1 | `config.py:533` |
| `weight-bits-positive` | `ConfigError` | ECC_WEIGHT_BITS <= 0 | `config.py:294` |
...
| id | raises | refuses | where |
|---|---|---|---|
| `panels-needs-arch-or-model-sweep` | `ConfigError` | a panel layout that would vary two axes at once | `config.py:266` |
| `recon-no-split-read-write` | `ConfigError` | ECC_SPLIT_READ_WRITE=1 inside the placement study | `config.py:412` |
| `rerun-needs-the-mapper` | `ConfigError` | ECC_RERUN_OPTIMISER=1 together with a knob that never invokes Timeloop | `config.py:462` |
