...
A STAGE THAT REFUSES IS SNAPSHOTTED, NOT SKIPPED. `run.sh recon --eval` at
env.sh's defaults refuses today, because Task 4's capacity cache is cold
(CLAUDE.md, "EVERY MAPPER CACHE IS COLD, ON PURPOSE"). That refusal is the
current behaviour of the code, so it is recorded with its exit status like any
other stage: a phase that changes it has changed behaviour, and the gate says so.
"""
import json
import os
import re
...
STAGES = (
    ...
    ("baseline",      ["bash", "run.sh", "baseline", "--eval"],
     {"ECC_PHASE": "Pre", "RECON_OPTIMIZER": "False"}),
