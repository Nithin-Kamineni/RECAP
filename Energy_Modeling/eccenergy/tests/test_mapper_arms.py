...
1. `test_mapper_arms_are_the_distinct_chips`
   `recon.mapper_arms()` is the reference plus every boundary that differs on
...
def test_mapper_arms_are_the_distinct_chips():
    ...
    def drop_no_ops():
        keys = {(a.narrow_levels, tuple(x for x in a.bw_scale if x[1] != "no-op"), a.ert)
                for a in recon.mapper_arms(ARCH_V2)}
    ...
    twin = dataclasses.replace(recon.placement_by_key(ARCH, "recon2"),
                               key="recon2b", variant="twin_of_recon2")
    real = recon.PLACEMENTS[ARCH]
    recon.WEIGHT_PATHS["_twin_test"] = recon.WEIGHT_PATHS[ARCH]
    recon.PLACEMENTS["_twin_test"] = real + (twin,)
    try:
        arms = recon.mapper_arms("_twin_test")
        _assert_eq(len(arms), len(recon.mapper_arms(ARCH)))     # NOT one more
        ...
        recon.PLACEMENTS["_twin_test"] = real + (
            dataclasses.replace(twin, site_counter="fills"),)
        _assert_eq(len(recon.mapper_arms("_twin_test")),
                   len(recon.mapper_arms(ARCH)) + 1)
    finally:
        del recon.PLACEMENTS["_twin_test"], recon.WEIGHT_PATHS["_twin_test"]
    ...
    renamed = tuple(dataclasses.replace(q, key=f"z{i}", variant=f"z{i}")
                    for i, q in enumerate(recon.PLACEMENTS[ARCH]))
    recon.WEIGHT_PATHS["_rename_test"] = recon.WEIGHT_PATHS[ARCH]
    recon.PLACEMENTS["_rename_test"] = renamed
    try:
        a_real = recon.mapper_arms(ARCH)
        a_fake = recon.mapper_arms("_rename_test")
        _assert_eq([(a.narrow_levels, a.bw_scale, a.ert) for a in a_fake],
                   [(a.narrow_levels, a.bw_scale, a.ert) for a in a_real])
    finally:
        del recon.PLACEMENTS["_rename_test"], recon.WEIGHT_PATHS["_rename_test"]
...
def test_a_network_stage_is_carried_and_marked_no_op():
    ...
    arms = {a.key: a for a in recon.mapper_arms(ARCH_V2)}
    ...
    kinds = {s.key: s.kind for s in recon.stages_for(ARCH_V2)}
    for key in recon.placement_by_key(ARCH_V2, "recon2").reduced:
        want = recon.BW_SCALE_TIMING[kinds[key]]
    ...
    fake = dataclasses.replace(
        recon.placement_by_key(ARCH_V2, "recon2"))
    stages = tuple(dataclasses.replace(s, kind="storage") if s.kind == "network" else s
                   for s in recon.stages_for(ARCH_V2))
    expect_raises(lambda: _assert_eq(recon.arm_bw_scale(fake, stages)[1][1], "no-op"),
...
def test_arm_slugs_are_distinct():
    ...
    # until Phase C1.2.
    def collide():
        got = [a.slug_part if a.ert else None for a in recon.mapper_arms(ARCH)]
...
def test_the_config_resolves_every_arm_and_gives_each_its_own_slug():
    ...
    arch = cfg.archs[0]
    seen = {}
    for arm in recon.mapper_arms(arch, cfg):
...
def test_no_bar_borrows_a_foreign_plan():
    ...
    cfg = config.load_config()
    arch = cfg.archs[0]
    got = recon.plan_assignment(arch, {"recon2", "recon4"}, cfg)
    ...
    arms = {a.key: a for a in recon.mapper_arms(arch, cfg)}
    ...
    everything = recon.plan_assignment(arch, set(arms), cfg)
    ...
    nothing = recon.plan_assignment(arch, set(), cfg)
...
def test_the_lender_is_the_outermost_candidate_and_the_others_are_recorded():
    ...
    cfg = config.load_config()
    arch = cfg.archs[0]
    rec = recon.plan_assignment(arch, {"recon2", "recon4"}, cfg)["recon3"]
    ...
    inner = recon.plan_assignment(arch, {"recon4"}, cfg)["recon3"]
...
def test_ert_condition3_is_derived():
    ...
    r5 = recon.placement_by_key(ARCH, "recon5")
    ...
    ok, why = recon.ert_injectable(r5, stages, 0.0)
    _assert_eq(ok, False)
    assert "constant" in why, why
    # ANY non-zero per-cycle row -> mapping-dependent -> injectable.
    ok, why = recon.ert_injectable(r5, stages, 0.0139456495)
    _assert_eq(ok, True)
    ...
    for pct in (0.0, 99.5, 100.0):
        ...
        rows[pct] = (recon.ert_leak_delta_pj(c),
                     tuple(p.key for p in recon.ert_arms(ARCH, c)))
    ...
    _assert_eq(recon.ert_injectable(renamed, stages, 0.0139456495)[0], True)
    _assert_eq(recon.ert_injectable(renamed, stages, 0.0)[0], False)
    expect_raises(lambda: _assert_eq(recon.ert_injectable(renamed, stages, 0.0)[0], True),
    ...
    _assert_eq(recon.ert_injectable(fills, stages, 0.0)[0], True)
    # BREAKAGE 3: a bar that is NOT on the innermost level is unaffected by the
    # leak row entirely.
    _assert_eq(recon.ert_injectable(recon.placement_by_key(ARCH, "recon2"), stages, 0.0)[0],
               True)
...
def test_ert_arms_and_mapper_arms_answer_different_questions():
    ...
    cfg = config.load_config()
    arch = cfg.archs[0]
    ert = {p.key for p in recon.ert_arms(arch, cfg)}
    mapper = {a.key for a in recon.mapper_arms(arch, cfg) if a.placement is not None}
    ...
    for key in sorted(mapper):
        spec = recon.mapper_arm_spec(arch, key, cfg)
        if key in ert:
            _assert_eq(recon.ert_arm_spec(arch, key, cfg)["level"], spec["ert"]["level"])
        else:
            expect_raises(lambda k=key: recon.ert_arm_spec(arch, k, cfg),
...
def test_the_cache_probe_separates_ready_cold_and_partial():
    ...
    import tempfile
    from ..experiments import recon as exp
    cfg = config.load_config()
    arch = cfg.archs[0]
    arm = [a for a in recon.mapper_arms(arch, cfg) if a.key == "recon2"][0]
    shapes = ["shapeA", "shapeB", "shapeC"]
    with tempfile.TemporaryDirectory() as tmp:
        root = pathlib.Path(tmp)
        ses = _FakeSession(root, shapes)
        _assert_eq(exp.arm_plan_state(cfg, ses, arch, "m", arm, shapes)[0], "cold")
        (root / "shapeA").mkdir()
        (root / "shapeA" / "timeloop-mapper.stats.txt").write_text("x")
        state, detail = exp.arm_plan_state(cfg, ses, arch, "m", arm, shapes)
        ...
        _assert_eq(exp.arm_plan_state(cfg, ses2, arch, "m", arm,
                                      shapes + ["shapeD"])[0], "partial")
        expect_raises(
            lambda: _assert_eq(exp.arm_plan_state(cfg, ses2, arch, "m", arm,
