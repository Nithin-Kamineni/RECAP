...
on the laptop; they passed on 2026-09-06. §7 is the parallel path (a SLURM job
array over architecture × model), built on 2026-09-06 once §5 had passed.
...
The unit of independent work is one **(architecture, model)** pair. Each pair
...
**Cost model, measured.** One (architecture, model) task maps every distinct
shape of that model serially at 18 threads. Timings from the 2026-09-06 run are
in FINDINGS.md § "HiPerGator bring-up". Scaling to the full eight-model
`ECC_SWEEP_MODELS` list is 221 distinct shapes × 6 architectures = 1,326
mapping tasks, which is a far larger commitment than the 43 × 6 = 258 of the
panel pair — size it against `slurmInfo -g rewetz` before submitting.
