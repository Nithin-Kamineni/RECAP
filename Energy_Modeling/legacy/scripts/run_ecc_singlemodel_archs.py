...
Fixes ONE model (default resnet18) and ONE BCH configuration (default K=57), then
sweeps every dataflow architecture in the exercises repo and draws them side by side
in a SINGLE image: one bar-group per architecture, each group holding three stacked
bars -- Baseline BCH / Embedded ECC / Patched ECC.
...
Same corrected per-level energy model as the k-sweep / multimodel scripts:
    ...
    Patched ECC   : ONLY the weight portion of on-chip energy scaled by K/N
                    (inputs/outputs keep full energy -- they carry no ECC);
...
K                  = 51                     # the ONE BCH(63,K) config to plot
BCH_N              = 63
WEIGHT_BITS        = 8
...
ax.set_title(f"{MODEL}  \u00b7  BCH({BCH_N},{K},{t})  \u00b7  "
...
png = FIG_DIR / f"allarchs_{MODEL}_K{K}{suffix}.png"
pdf = FIG_DIR / f"allarchs_{MODEL}_K{K}{suffix}.pdf"
...
for a in archs:
    ...
    rows[a] = row
pd.DataFrame(rows).T.to_csv(FIG_DIR / f"allarchs_{MODEL}_K{K}{suffix}_summary_uJ.csv")

print(f"\nDone. Saved into {FIG_DIR}:")
print(f"  {png.name}")
print(f"  {pdf.name}")
print(f"  allarchs_{MODEL}_K{K}{suffix}_summary_uJ.csv")
for a in archs:
