...
## 15. Task 3 — Eyeriss v2 reconstruction placement, fixed mapping (2026-09-07)

**Headline: R4b — reconstruct at the weight-scratchpad output and retain what
was rebuilt — is the best boundary, and it beats embedded-only ECC by 1.36 %
(6.75 % against conventional ECC).** It captures 65.3 of the 77.5 µJ that the
reducible weight path makes available, i.e. 84 % of the theoretical ceiling. The
ordering the source discussion's ratings predict holds: R4b (5/5) beats R2/R3
(4/5), which are break-even, which beat R4a (3/5), which is 18 % worse than
doing nothing.

**A first version of this model reported R4b as the WORST boundary. That was a
bug in the reuse model, not a result — see "the bug" below.** The numbers in
this section are the corrected ones.

### The weight path as modelled (full resnet18, weight energy only)

`eyeriss_v2_like` has **no weight GLB** — its GLB banks are iacts and psums
(JETCAS 2019 Table II) — so the path is four stages, not five, and
`Global buffer` weight energy is exactly 0. (`eyeriss_v2_like_wglb` is the
variant that keeps weights in a GLB; it has no mapper cache yet, so its
upstream boundaries are unmeasured.)

| stage | Timeloop level | weight energy | wire | switching | accesses (TOTAL) | resident/PE | phys. word | reducible |
|---|---|---|---|---|---|---|---|---|
| `dram` | `DRAM` | 886.387 µJ | — | — | 13,849,792 reads | — | 64 b | **no** (ECC) |
| `inter_cluster_mesh` | `NoC: inter_PE_cluster_spatial <==> …` | 24.158 µJ | 19.289 | 4.870 | 13,849,792 ingresses | — | 8 b | yes |
| `cluster_local` | `NoC: inter_PE_spatial <==> ifmap_spad` | 13.694 µJ | 6.749 | 6.945 | 21,895,552 ingresses | — | 8 b | yes |
| `weight_spad` | `weights_spad` | 369.276 µJ | — | — | 1,814,073,344 reads / 21,895,552 fills | 16–256 | 24 b | yes |

### The ceiling, which every result has to be read against

Every result file and every console run now prints this first, because a 0.06 %
number is otherwise indistinguishable from a missing term:
...
So on this architecture the reduced representation is worth **at most 1.62 % of
inference energy**, and 91 % of that lives in the PE scratchpad. Not because the
K/N reduction is not applied — it is, to every reducible stage — but because
Eyeriss v2's on-chip weight path is only 8.5 % of its inference energy, its two
weight networks are 0.79 %, and its DRAM weight traffic (18.5 %) may not be
reduced at all.

### The five boundaries

BCH(63,51), 8-bit weights, `stream` packing, `weight` encoder charging,
`tile`-sized reuse register, reconstruction 4.1296 pJ/codeword
(`data/dc/BCH_N63_results.json`, `BCH_63_51_t2`, incremental + idle):

| bar | total | vs conventional ECC | vs embedded only | recon | overhead | N_rec (codewords) |
|---|---|---|---|---|---|---|
| Baseline (external parity) | 5,075.371 µJ | — | −5.773 % | — | — | — |
| Embedded (no external parity) | 4,798.375 µJ | +5.458 % | — | — | — | — |
| R1 @ NoC source | 4,805.637 µJ | +5.315 % | −0.151 % | 7.263 µJ | — | 1,758,704 |
| R2 @ cluster edge | 4,801.036 µJ | +5.405 % | −0.055 % | 7.263 µJ | — | 1,758,704 |
| R3 @ SPad input | 4,802.647 µJ | +5.373 % | −0.089 % | 11.482 µJ | — | 2,780,388 |
| R4a @ SPad output | 5,672.121 µJ | −11.758 % | −18.209 % | 951.295 µJ | — | 230,358,520 |
| **R4b @ SPad + reuse reg** | **4,733.027 µJ** | **+6.745 %** | **+1.362 %** | 11.482 µJ | 0.718 µJ | 2,780,388 |

Signs are savings; a negative number costs *more* than the reference.
R4b reconciles by hand: `407.128 × 0.1905 = 77.55 µJ` saved, `11.482 + 0.718 =
12.20 µJ` paid, `4798.375 − 77.55 + 12.20 = 4733.03 µJ`.

R1 being *worse* than embedded is correct and is why it is rated 2/5: it strips
the bits and immediately puts them back, so it buys nothing on chip and pays the
encoder. It is in the study as the control that isolates reconstruction cost
from every benefit.

### The bug: R4b's reuse model asked the wrong question

The first version asked **how many uses one weight gets IN A ROW**, i.e. what a
one-entry latch can serve. Measured from every mapping's loop nest, that is
**1**: the innermost temporal loop below `weights_spad` is `M` on all 12
resnet18 shapes (and `G`/`M`/`R` on mobilenet_v2), all weight dimensions, so the
weight changes every iteration. R4b therefore came out identical to R4a plus a
register it never used, i.e. the worst boundary of the five — inverting the
source discussion's 5/5 rating and contradicting the plain physical argument
that a retained reconstruction must need fewer reconstructions than an
unretained one.

That was an artifact of the question. `01_project_context_and_architectures.txt`
§5.4's EV2-C is not a latch that must see back-to-back uses; it is
**"Reload/reconstruct when the required weight CHANGES"** — a retained
reconstruction that survives until the value it holds is replaced. The right
question is a *capacity* question, and it is answered from the mapping:

| measured, all 12 resnet18 shapes | |
|---|---|
| loops below `weights_spad` walk `inner_tile` distinct weights | 16 – 256 |
| `inner_tile` == the buffer's utilized capacity | **true for every shape** |
| that walk repeats `reads / fills` times before a refill | 1 – 6272 (82.85× overall) |
| consecutive uses of one weight (what a latch serves) | 1 |
| innermost weight dimension | `M` — the *slowest* index of the `[M][C][R][S]` stream order, so consecutively accessed weights are never consecutive in the codeword bit stream, and a group-sized buffer catches nothing either |

So a register holding the working set reconstructs each weight **once per
buffer fill** — `N_rec = fills = 21,895,552 weights`, exactly R3's count — while
keeping the scratchpad reduced. **R4b is R3's encoder count with R4a's storage
saving**, which is precisely why the source discussion rates it above both. The
amortization against R4a is 82.85×.

A register *smaller* than the working set catches nothing, and that is not a
simplification: the access pattern is a cyclic walk of period `inner_tile`,
which is the textbook LRU worst case, so there is no partial hit rate. This is
now reachable as `ECC_RECON_REUSE_REG_ENTRIES=1` (a latch) or `=9` (a group
buffer) — both collapse R4b back onto R4a, at −19.45 %.

`eccenergy/recon.py::retention_model()` carries this reasoning in full, and
`recon.weight_loop_nest()` now reports `inner_tile`, `consecutive_run`,
`innermost_weight_dim` and stream-consecutiveness separately instead of running
them together.

### R4b's real overhead, which is capacity, not energy

The register must hold up to **256 full-width weights per PE** (max over the 21
layers; 16 at the smallest). Against the published 288-weight scratchpad that
is not "small". Per resident weight the PE now holds 6.48 b of reduced
scratchpad **plus** 8 b of register = 14.48 b, against the baseline's 8 b —
**1.81× the PE weight storage.** So R4b's honest description is *"move the
full-width storage into a cheap register and keep the scratchpad reduced"*: it
buys access-bit-volume, not capacity. The energy model charges only the register
writes (0.718 µJ); the area and leakage of those entries are not modelled, and
Task 4 is where the plan asks for area.

The register reads are deliberately **not** charged. Physically they replace
scratchpad reads, and scratchpad read counts are held at Timeloop's under every
bar (no operand-retention saving is credited to anybody, which is the comparison
the plan asks for), so charging a register read on top would bill the same
delivery twice. A register read (0.033 pJ) is cheaper than a scratchpad read
(0.20 pJ), so this over-charges R4b rather than under-charging it.

### mobilenet_v2: the same architecture, two boundaries rejected

`G_rec = 9` weights at BCH(63,K) over 8-bit weights. All 12 resnet18 shapes keep
≥ 16 weights resident, so all five boundaries are feasible. On mobilenet_v2, 7
depthwise layers keep only **3** resident (`C1_M1_R3_S3_…_G*`), so R4a and R4b
are `unsupported` with those layers named — the retained bits they would rebuild
from were never delivered to that PE.

| mobilenet_v2 | total | vs conventional | vs embedded |
|---|---|---|---|
| Baseline | 1,862.971 µJ | — | −3.938 % |
| Embedded | 1,792.380 µJ | +3.789 % | — |
| R1 | 1,794.231 µJ | +3.690 % | −0.103 % |
| R2 | 1,793.512 µJ | +3.728 % | −0.063 % |
| R3 | 1,793.483 µJ | +3.730 % | −0.062 % |
| R4a / R4b | — | `unsupported` | 7 layers hold 3 < G_rec = 9 |

Its ceiling is lower again (66.8 µJ reducible = 3.72 % of the total; 0.71 % after
K/N), because depthwise layers have little weight reuse to spend scratchpad
energy on.

### Sensitivity — `vs embedded`, in %, resnet18 full model

| | R1 | R2 | R3 | R4a | R4b |
|---|---|---|---|---|---|
| **default** (`stream`, `weight`, reg=`tile`) | −0.15 | −0.06 | −0.09 | −18.21 | **+1.36** |
| `ECC_RECON_PACKING=aligned` | −0.15 | −0.10 | −0.17 | −19.76 | **−0.19** |
| `ECC_RECON_ENCODER_GRANULARITY=codeword` | −1.19 | −1.10 | −1.73 | −154.51 | −0.28 |
| `ECC_RECON_REUSE_REG_ENTRIES=1` (a latch) | −0.15 | −0.06 | −0.09 | −18.21 | −19.45 |
| `ECC_RECON_REUSE_REG_ENTRIES=9` (group buffer) | −0.15 | −0.06 | −0.09 | −18.21 | −19.45 |
| `ECC_RECON_INCLUDE_IDLE=0` | −0.07 | +0.03 | +0.04 | −7.50 | **+1.49** |

Three things this says, and they are the caveats that matter most:

* **R4b's whole benefit is contingent on physical packing.** Under `aligned`
  packing — `ceil(8 × 51/63) = 7` bits per weight, `floor(24/7) = 3` values per
  24-bit scratchpad word, the same 3 as at 8 bits — the scratchpad access count
  does not move and R4b's +1.36 % becomes −0.19 %. §16 of file 02 is exactly
  right, and the `stream` default is defensible only because the embedded layout
  genuinely is a packed bit stream (`embedded.py`): the retained portion of a
  63-bit codeword is its 51 *contiguous* message bits, with no per-weight
  alignment to preserve. **A real implementation must pack across weight
  boundaries or R4b buys nothing.**
* **R4b's benefit is contingent on the register covering the working set.** At 1
  or 9 entries it collapses to −19.45 %. The 5/5 rating is a claim about a
  tile-sized register, not about a latch.
* **The encoder's idle term matters more than the ordering.** 4.1296 pJ/codeword
  is 1.8995 incremental + 2.2301 idle-per-cycle, i.e. one cycle of standing
  power per codeword. Dropping it lifts R4b to +1.49 % and lifts R2/R3 from
  break-even to a (tiny) positive saving. It does not change any ordering.

### Checks that passed

Every Task 3 check: `weight_path_reconciles_with_raw_record` (this module's own
re-parse of the cached stats reproduces the `Raw` record's weight energy per
category, exactly), `dram_identical_to_embedded_reference`,
...
`evaluation_only_rerun`, `reference_bars_match_tasks_1_and_2`,
`fixed_mapping_shared_across_variants` and
`placement_space_covers_the_whole_weight_path` (added by the second-pass review
below), plus Task 1's inherited set.
`eccenergy/tests/test_recon.py`: **26/26**, including the two that pin the
corrected R4b behaviour in both directions and the seven property tests the
review added, which run on the real mapper cache as well as the synthetic
stats.

`baseline_external_parity` = 5,075.3712 µJ and `embedded_ecc` = 4,798.3746 µJ
are identical to the Task 1 and Task 2 files to four decimal places, before and
after this session.

The one FAIL, `noc_share_within_published_band_eyeriss_v2` (modelled 4.579 % vs
the published 6–10 % band), is **pre-existing**: the Task 1 and Task 2 files for
the same (arch, model) fail it too.

### Caveats that stand

* **Fixed mapping.** No capacity-driven reuse and no re-tiling is credited.
  Packed reduced weights raise the *effective* weight capacity, which is Task
  4's question. Note the direction this cuts: R4b already needs 1.81× the PE
  weight storage, so a reconstruction-aware mapper has less headroom than the
  reduced scratchpad alone suggests.
* **CSC metadata is not modelled.** The published v2 PE stores weights
  CSC-compressed with a separate address scratchpad (Table IV: weight data
  288 B, weight address 16×7 b); this study models the design dense. A CSC
  implementation would reduce the weight *data* only, so R4a/R4b's scratchpad
  saving is an upper bound. It is a warning on every result file.
* **Register area and leakage are not modelled**, only its write energy. The
  required capacity is reported per layer so the omission is visible.
* Whole-model, one architecture, one network, one code. Not an architecture
  ranking; §13's convergence caveats still apply.
* Observed while parsing the NoC stats and **not fixed**:
  `archs/_shared/noc.yaml` declares `eyeriss_v2_like`'s intra-cluster level
  `PE: {router_pj: 0.0}`, but the flattened architecture Timeloop is given
  carries `router_energy: 0.0833` on `inter_PE_spatial`, so 6.945 of the
  cluster-local stage's 13.694 µJ is switching. It affects every bar equally and
  scales identically under `stream`, so it does not bias this comparison — but
  the declaration and `archs._inject_noc` disagree and one of them is wrong.
  **The second-pass review below found the mechanism** (`_inject_noc` writes
  `router_energy` only `if router:`, so a declared zero is never emitted and the
  level inherits its parent's value), **sized it** (44.94 µJ, 20.5 % of the whole
  NoC) and states why it must wait for the next cold mapping pass.
* **The memory-controller-to-accelerator link is not modelled at all**, and the
  886.387 µJ DRAM term cannot be split to expose it: Accelergy's `CactiDRAM` is a
  flat 8 pJ/bit LPDDR4 constant using only `(type, width)`. See the review below
  for what a split would be worth and why no boundary in this study would claim
  it anyway.

### Review, second pass (2026-09-07): is the reduction applied, and is it the right size?

The question put to this pass: *"why is there no visible energy reduction going
from embedded to the reconstruction approaches — at R2 the mesh should carry
only K/N of the data, at R3 the mesh and the cluster-local fanout both should."*
Two separate claims, answered separately.

#### (A) The reduction IS applied, correctly and completely — PROVED, not asserted

It is applied to every stage a boundary reduces, to every digit a double can
carry, and to no other stage. Measured on the real cached Timeloop output for
`eyeriss_v2_like` / resnet18 (full model, 21 layers, 12 shapes):

| stage | weight energy | R1 | R2 | R3 | R4a | R4b |
|---|---|---|---|---|---|---|
| `dram` (not reducible) | 886.387 µJ | = | = | = | = | = |
| `inter_cluster_mesh` | 24.158 µJ | = | **19.557** | **19.557** | **19.557** | **19.557** |
| `cluster_local` | 13.694 µJ | = | = | **11.085** | **11.085** | **11.085** |
| `weight_spad` | 369.276 µJ | = | = | = | **298.938** | **298.938** |
| **category `NoC`** | 219.718 µJ | 219.718 | **215.117** | **212.508** | 212.508 | 212.508 |
| **category `Local`** | 631.838 µJ | 631.838 | 631.838 | 631.838 | **561.499** | **561.499** |

Every bold entry is `before × K/N` to within 4e-16 relative — i.e. exactly, in
double precision. Every `=` is bit-identical, not merely close. So both halves
of the question are answered yes: at R2 the mesh does fall by
`24.158 × (1 − 51/63) = 4.602 µJ`, and at R3 the cluster-local fanout falls by a
further `13.694 × 0.190476 = 2.608 µJ` on top of it. **The savings are there;
they are 0.096 % and 0.150 % of a 4,798 µJ bar.**

Seven property tests now pin this, each run on BOTH the synthetic stats.txt the
suite builds AND the real mapper cache the published figure was drawn from
(`eccenergy/tests/test_recon.py`, now **26** tests):

| test | what would break it |
|---|---|
| `every_reduced_stage_scales_by_k_over_n_and_no_other_stage_moves` | a reduction dropped, mis-scaled, or applied to an untouched stage (untouched stages are compared with `==`, not `isclose`) |
| `only_the_weight_share_of_a_category_moves` | an iact/psum/compute share moving inside `NoC` or `Local` |
| `category_energies_are_monotonic_down_the_weight_path` | *"R3's NoC is not below R2's"* — the exact failure the question describes |
| `the_saving_of_each_boundary_matches_its_closed_form` | the identity `NoC(R3) == NoC(emb) − (mesh + cluster_local)×(1−K/N)`, not an inequality |
| `no_stage_or_transport_category_exceeds_the_embedded_reference` | any silent credit, including a DRAM one |
| `a_stronger_code_saves_strictly_more_at_every_reduced_stage` | a reduction applied but not keyed to the CODE (K swept over 57/51/45/39/36/30) |
| `every_weight_carrying_level_of_the_real_design_is_claimed_by_a_stage` | a weight-carrying level no stage claims — checked by an INDEPENDENT re-scan of the cached stats, not by re-using the module's own parser |

**Mutation-tested, because a property test that cannot fail proves nothing.**
Six deliberate breakages of `recon.py`, each reverted immediately:

| mutation | tests that caught it |
|---|---|
| `Packing.storage_scale` forced to 1.0 (SPad reduction dropped) | 7 |
| network wire/switching scales forced to 1.0 | 5 |
| `Packing.frac` hard-wired to 51/63, ignoring K | **1 — the code sweep alone** |
| R2 reduces `cluster_local` instead of the mesh (right size, wrong stage) | 2 |
| a silent 1 % DRAM credit | 4 |
| the mesh saving applied twice | 4 |

The third row is the one worth noting: a reduction that is applied but not
actually keyed to the code passes every other test in the suite.

Independently confirmed from the raw record: exactly four levels on this design
spend Weights energy — `DRAM`, `NoC: inter_PE_cluster_spatial <==> …`,
`NoC: inter_PE_spatial <==> …` and `weights_spad` — and all four are claimed by
exactly one stage each. `iact_glb` holds Inputs only and `psum_glb` Outputs
only, so **the 0.00 µJ weight GLB is a fact about the design, not a gap.**

#### A latent bug this pass DID find, and fixed: `eyeriss_v2_like_wglb`

`WEIGHT_PATHS["eyeriss_v2_like_wglb"]` inserts a reducible `weight_glb` stage
between DRAM and the mesh. `PLACEMENTS["eyeriss_v2_like_wglb"]` is
`eyeriss_v2_like`'s five boundaries **unchanged**, and not one of them reduces
it. Had that design ever been run, R2/R3/R4a/R4b would each have reported the
weight GLB at FULL width while still claiming their own savings — every
boundary understated, and nothing in the result file saying so. `feasibility()`
structurally cannot see it: it inspects the stages a placement *does* claim,
never the ones it should have claimed and did not.

`recon.validate_placement_space(arch)` now enforces the invariant the physics
implies — the reduced form is contiguous from the ECC engine down to the
boundary, so a placement's `reduced` set must be a **prefix** of the path's
reducible stages in path order, and every reducible stage must be reached by
some boundary. It is recorded as `placement_space_covers_the_whole_weight_path`
on every result and raises before evaluation in `experiments/recon.py::run()`.
`eyeriss_v2_like` passes (all five sets are prefixes); `eyeriss_v2_like_wglb`
now fails loudly with the five violations named. Task 5 fixes it by adding the
GLB boundary alongside the stage, which is what CLAUDE.md already says to do.
**No `eyeriss_v2_like` number moves: the console output of `recon --eval` is
byte-identical before and after.**

#### (B) The magnitude — the three candidate missing terms, in the order asked

**1. The DRAM-to-accelerator link. Cannot be separated, and it is the model's
largest stated limitation.** Timeloop's single DRAM level is costed by
Accelergy's `CactiDRAM` plug-in, whose entire energy model is

```python
# /usr/local/share/accelergy/estimation_plug_ins/accelergy-cacti-plug-in/cacti_wrapper.py:45
class CactiDRAM(Estimator):
    type2energy = {"LPDDR4": 8, ...}          # pJ per bit; comment: "Public data"
    def read(self):  return self.type2energy[self.type] * 1e-12 * self.width
```

and whose own log confirms it: *"Arguments used: (type, width)"* — depth, banks,
datawidth and technology are all discarded. So the 512 pJ per 64-bit word is a
**flat 8 pJ/bit with no array/I-O decomposition anywhere in it**. There is no
link component to peel off; splitting it would mean importing an external
LPDDR4 breakdown, which is a guess with respect to this model's own numbers.
Recorded as a limitation, not estimated. Note also that Timeloop charges the
DRAM↔chip network **zero** (no spatial fanout ⇒ no hops, and v2's
`ingress_pj` is 0), so the link is not merely bundled — it is absent.

What it would be worth if it *were* separable, so the omission has a size:

| link fraction of the 886.387 µJ | extra saving at 1−K/N | ceiling on ANY boundary |
|---|---|---|
| 0 % (as modelled) | — | 77.5 µJ = **1.62 %** |
| 10 % | 16.9 µJ | 94.4 µJ = 1.97 % |
| 25 % | 42.2 µJ | 119.8 µJ = 2.50 % |
| 50 % | 84.4 µJ | 162.0 µJ = 3.38 % |

**And even then, no boundary in this study would claim it.** Section 1 of
`01_project_context_and_architectures.txt` puts the ECC check/correction between
the DRAM read and the accelerator datapath, and R1–R4b all place their encoder
at or below the accelerator's weight-NoC ingress — downstream of the link. A
boundary that shrinks the link is *"correct at the memory controller and ship
K/N bits across"*, which is a sixth boundary this study does not have and a
different partitioning assumption (the corrector leaves the accelerator). It
would also apply equally to R1–R4b, i.e. it is a constant offset on every bar,
not a discriminator between them. **Task 5 material, flagged here so it is not
mistaken for an oversight.**

**2. The weight NoC is not under-counted in any way that matters, and the
`noc_share` FAIL is probably a DENOMINATOR question.** Three findings:

* *Multicast.* The premise that weights are scattered where they should be
  multicast holds for one shape, not the design. Across the 12 resnet18 shapes
  the mesh's weight multicast factor is 1, 2 or **4** (`C64_M64_R3_S3_P56_Q56`:
  fanout 16, multicast 4, 13.5 hops). Weight ingresses at the mesh equal the
  DRAM weight reads exactly (13,849,792), which is the floor: every weight
  delivered is charged once. There is no scatter/multicast error to correct.
* *Magnitude has almost no leverage anyway.* The weight share of the NoC is
  37.85 of 219.72 µJ, i.e. **0.79 % of the run**, and the two networks are 9.3 %
  of the 407 µJ of reducible weight energy. Scaling the whole NoC up to the
  published band moves the ceiling by a rounding error:

  | NoC | share (incl. DRAM) | weight NoC | ceiling |
  |---|---|---|---|
  | as modelled | 4.58 % | 37.85 µJ | 77.55 µJ = 1.616 % |
  | ×1.330 → bottom of the band | 6.00 % | 50.35 µJ | 79.93 µJ = 1.641 % |
  | ×2.315 → top of the band | 10.00 % | 87.64 µJ | 87.03 µJ = 1.711 % |

  So even a doubled interconnect leaves R2 at 0.13–0.22 % instead of 0.096 %.
  **The NoC is not why the figure looks flat.**
* *The band check's denominator.* `audit.py` computes
  `noc_share = NoC / raw.total`, and `raw.total` **includes DRAM**. Against
  on-chip energy alone the modelled share is
  `219.718 / (4798.375 − 1433.441) = ` **6.53 %**, which is inside JETCAS 2019
  Sec. V's 6–10 % band. Which denominator the paper's Fig. 18 uses is a factual
  question about the paper and is **not settled here** — there is no copy of it
  in this repo — but it is the first thing to check before touching a
  coefficient, because the same class of error (a mis-scoped share) produced the
  15 % and 30 % figures this model already corrected. Recorded, verdict
  unchanged: the check is Task 1's and Task 1's code is frozen.

**A real defect confirmed while measuring this, and deliberately NOT fixed.**
`archs/_shared/noc.yaml` declares `eyeriss_v2_like`'s intra-cluster level
`PE: {router_pj: 0.0}`, but `archs._inject_noc` emits `router_energy` only
`if router:` — so a declared **zero is never written**, and the level inherits
its parent's 0.0833 pJ. The flattened architecture confirms it:
`inter_PE_spatial[1..16]` carries `router_energy: 0.08333`. The docstring's own
warning ("absence is not enough") is applied to the datapath branch and not to
this one. Size of it: **44.94 µJ**, 20.5 % of the whole NoC and 0.94 % of the
run; 6.80 µJ of that is weight energy. Correcting it would put the NoC at
174.78 µJ (3.68 % incl. DRAM) — *further below* the published band — and cut
R3's saving from 7.210 to 5.914 µJ. **Not fixed, for two stated reasons:** it
changes the patched YAML text, therefore the mapping fingerprint, therefore the
entire `eyeriss_v2_like` mapper cache (a cold run — §"the cost of mapping"
below); and it changes Tasks 1 and 2's frozen 5,075.3712 / 4,798.3746 µJ, which
this task is required to leave alone. It is a one-line change (`if router:` →
always emit) whose correct home is the next cold mapping pass.

**3. The GLB.** `eyeriss_v2_like` genuinely has no weight GLB — confirmed above
from the raw record, level by level — so 0.00 µJ is right for this design and
Sec. 5.1's warning is being honoured, not overlooked. The bracketing variant
`eyeriss_v2_like_wglb` is the one that would measure a global-buffer boundary,
and it is **entirely unmeasured**: no mapper cache, and (until this pass) a
placement list that would have understated it anyway.

*What a cold map would cost, measured rather than guessed.* From the SLURM log
of the array that produced the current cache (job `41296638`, task 0, the same
settings this study uses — energy objective, victory 2000 `levels`,
`random_pruned`, **uncapped** search, timeout 2000 s, 18 threads):

| design × model | shapes | wall time, 18 cores |
|---|---|---|
| `eyeriss_v2_like` × resnet18 | 12 | **2 h 26 m** |
| `eyeriss_v2_like` × mobilenet_v2 | 31 | 3 h 00 m |
| `eyeriss_like` × resnet18 | 12 | 1 h 03 m |

`eyeriss_v2_like_wglb` adds one storage level, and `ECC_VICTORY_SCALING=levels`
raises the search budget with nest depth while the mapspace itself grows
combinatorially — so it is a **lower** bound: budget 2.5–4 h wall (≈45–70
core-hours) for resnet18 alone, one array task, and roughly the same again if
mobilenet_v2 is wanted. **Worth doing** — it is the only way to measure a
global-buffer boundary at all, and the GLB category is 8.19 % of this run
today — but it must be quoted as a BOUND, not as Eyeriss v2: v2's extra weight
level is not in its paper (CLAUDE.md, "Eyeriss v1 comes as a bracketing PAIR").
**Not launched: a cold mapping run needs an explicit go-ahead.**

#### The verdict on the two claims

* **(A) held.** The reduction is applied to every reducible stage of every
  boundary, exactly, and to nothing else. The questioner's arithmetic is right
  and so is the model's; what was missing was a way to see 4.6 µJ on a 4,798 µJ
  bar.
* **(B) did not hold as stated.** The five weight-path numbers are right for
  what the model contains: the DRAM figure is a flat 8 pJ/bit with no separable
  link, the weight NoC is not under-counted (and would barely matter if it
  were), and the zero weight GLB is a property of the design. The one term that
  could move the answer materially — the memory-controller-to-accelerator
  link — is not in the model at all and is not reachable by any of the five
  boundaries; it is now a stated limitation with its size shown.
* **So the headline stands:** on a dense 45 nm Eyeriss v2 running resnet18, the
  reduced representation is worth **at most 1.62 %** of inference energy, and
  R4b captures 84 % of that. It is a small number because the on-chip weight
  path is 8.5 % of the run, not because a term is missing.

#### The figure, which could not show any of this

A 4.60 µJ change on a 4,798 µJ stacked bar is about one pixel at any sane figure
size, so `ReconSweep.png` showed five boundaries that looked identical to
embedded. The model was right, so the figure was widened rather than the model
changed — `plots/stacked.py::draw_panel` gained two parameters and stayed the
only routine in the project that draws a bar:

* `bar_notes` prints one caller-supplied line per bar. The placement figure puts
  the **µJ moved on chip and the µJ paid for reconstruction** there, because a
  percentage cannot separate R2's 0.096 % from R3's 0.150 % and those two
  numbers are the result.
* There was a **second panel** underneath, drawn by `grouped_stacks(zoom_stacks=…)`
  calling `draw_panel` again over the same groups, palette and legend but its
  own energy unit, scoped to the **on-chip weight path**: 407.1 µJ full-width,
  402.5 at R2, 399.9 at R3, 336.8 at R4a/R4b. **It was removed on 2026-09-07**
  and `zoom_stacks` with it. On `eyeriss_v2_like` that panel resolved to exactly
  two bands — `weight_spad` at 369.28 µJ and the two networks at
  24.16 + 13.69 = 37.85 µJ — i.e. the weight share of `Local (spads/RF)` and
  `NoC`, both of which the panel above already draws. So it restated the upper
  bars at a 12× scale rather than adding a quantity, and it was the largest
  thing in the image. The numbers it carried are still on the record: the per-bar
  note gives the µJ moved on chip, and `reducible_energy_uJ` and
  `saving_ceiling_uJ` are columns of `ReconSweep.csv` that every console run
  also prints as the CEILING block.

Still one image and one file (`results/figures/ReconSweep.{png,pdf}`), one table,
one manifest. **The three sweeps are untouched:** `bash run.sh --replot` and
`ECC_SWEEP=bch … --replot` reproduce `ArchitectureSweep.png` and `BCHsweep.csv`
**byte-for-byte** under the new `draw_panel` (md5 checked before and after).
...
# the other point and the sensitivity corners
...
ECC_RECON_MODELING=1 ECC_RECON_REUSE_REG_ENTRIES=1          bash hpc/tl.sh bash run.sh recon --eval
ECC_RECON_MODELING=1 ECC_RECON_INCLUDE_IDLE=0               bash hpc/tl.sh bash run.sh recon --eval

bash hpc/tl.sh python3 -m eccenergy.tests.test_recon        # 26 tests; the seven
                                                            # property ones also run
                                                            # against the real cache
```

Figure `results/figures/ReconSweep.{png,pdf}`, table
`results/tables/ReconSweep.csv` (per-category energies, savings against BOTH
references, reconstruction counts, retention amortization, the reducible-energy
ceiling, and a row for every boundary including the unsupported ones), manifest
`results/manifests/ReconSweep.json`, result files
`results/evaluation/Pre/eyeriss_v2_like/<model>/bch63_51/w8a8/layers-full/map-energy-vic2000l-random_pruned-seednone-ssnone-perm16/*.json`
